...
Six collectors run hourly and write Parquet captures with their capture
...
station an open market's resolution source names (Phase 17), and the
point-in-time forecasts from three days back to tomorrow at the same
stations, keeping only rows already final. That last one began daily
(migration 0019) and moved to the hourly schedule (migration 0020), since
the archive assumes an hour's fetch latency (`docs/evidence.md`); a
station that fails does not stop the others. Every capture now has a manifest beside it (`<stamp>.json`: rows,
