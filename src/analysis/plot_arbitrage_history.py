...
def plot_arbitrage_history() -> None:
    """
    Plot the historical arbitrage spread between Polymarket and Kalshi.
    """
    ...
    for m in poly_event.get("markets", []):
        team: str = str(m.get("groupItemTitle", m.get("question"))).lower()
    ...
    for m in kalshi_markets:
        ...
        kalshi_map[team_clean] = m.event_id

    common_teams: Set[str] = set(poly_map.keys()) & set(kalshi_map.keys())
    ...
    for team in common_teams:
        ...
        # Poly History
        poly_hist: List[Dict[str, Any]] = poly_collector.fetch_price_history(
            poly_map[team], interval="max"
        )
        # Kalshi History
        kalshi_hist: List[Dict[str, Any]] = kalshi_collector.fetch_candlesticks(
            kalshi_series, kalshi_map[team], start_ts=start_ts
        )
        ...
        def get_k_price(row: pd.Series) -> float:
            p = row.get("price", {})
    ...
    filename = (
        f"plots/cs2_starladder_budapest_major/arbitrage_history_{ts_filename}.png"
    )
