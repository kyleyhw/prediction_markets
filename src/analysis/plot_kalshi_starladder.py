...
def plot_kalshi_starladder():
    ...
    for m in markets:
        ...
        title = m.event_name
        team_name = title.replace("Will ", "").replace(" win the StarLadder Budapest Major 2025?", "").strip()
        
        print(f"Processing: {team_name} ({m.event_id})")
        
        candles = collector.fetch_candlesticks(series_ticker, m.event_id, start_ts=start_ts)
    ...
    plt.title(f"Kalshi Odds History: StarLadder Budapest Major 2025")
    plt.xlabel("Date")
    ...
    filename = "plots/cs2_starladder_budapest_major/kalshi_winner_odds.png"
